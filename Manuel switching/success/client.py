...
def main():
    ...
    while True:
        ...
        camera_id = int(input("Enter camera id to be set(between 0 to 3): "))
        cam_width = int(input("width at which video has to be captured: "))
        cam_height = int(input("height at which video has to be captured: "))  
        test_json = json.dumps({"ID":camera_id,"WIDTH":cam_width,"HEIGHT":cam_height})
        try:
            my_socket.send(test_json.encode('utf-8'))
            data_sent = True
            wait_end_feed = True
            print("sent",camera_id,cam_width,cam_height)
