...
def main():
    ...
    # Run the command and raise an exception if it returns a non-zero status code
        subprocess.run(cmd1, stderr=subprocess.DEVNULL, shell=True, check=True)
    except:
        # Handle the exception
        print("slip file already exixts")

    while True:
        result = subprocess.run('ls /dev/ttyUSB*', stdout=subprocess.PIPE, encoding='utf-8', shell=True)
            ...
            while True:
                ...
                if data:
                    ...
                    while(True == wait_end_feed):
                        pass
                    command_runner_threaded(f"gst-launch-1.0 v4l2src device=\"/dev/video{camera_id}\" ! video/x-raw, width={cam_width}, height={cam_height}, framerate=10/1 ! videoconvert ! 'video/x-raw, format=I420, width={cam_width}, height={cam_height}' ! omxh265enc bitrate=40 iframeinterval=5 EnableStringentBitrate=true control-rate=4 temporal-tradeoff=4 EnableTwopassCBR=true ! 'video/x-h265, stream-format=byte-stream' ! h265parse ! queue ! rtph265pay pt=127 ! udpsink host=10.0.0.2 port=6000 sync=false", stop_on=trigger_function, on_exit=feed_ended, priority='low', io_priority='low',shell=False)
                else:
