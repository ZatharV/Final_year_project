...
def main():
    ...
    command_runner_threaded(f"gst-launch-1.0 v4l2src device=\"/dev/video0\" ! video/x-raw, width=160, height=120, framerate=10/1 ! videoconvert ! 'video/x-raw, format=I420, width=160, height=120' ! omxh265enc bitrate=40 iframeinterval=5 EnableStringentBitrate=true control-rate=4 temporal-tradeoff=4 EnableTwopassCBR=true ! 'video/x-h265, stream-format=byte-stream' ! h265parse ! queue ! rtph265pay pt=127 ! udpsink host=10.0.0.2 port=6000 sync=false v4l2src device=\"/dev/video1\" ! video/x-raw, width=160, height=120, framerate=10/1 ! videoconvert ! 'video/x-raw, format=I420, width=160, height=120' ! omxh265enc bitrate=40 iframeinterval=5 EnableStringentBitrate=true control-rate=4 temporal-tradeoff=4 EnableTwopassCBR=true ! 'video/x-h265, stream-format=byte-stream' ! h265parse ! queue ! rtph265pay pt=127 ! udpsink host=10.0.0.2 port=6001 sync=false", stop_on=trigger_function, on_exit=feed_ended, priority='low', io_priority='low',shell=False)
    
    while True:
        global camera_id, cam_width, cam_height
        try:
            ...
            while True:
                ...
                if data:
                    ...
                    camera_id = int(received_dict['ID'])
                    # cam_width = str(received_dict['WIDTH'])
                    # cam_height = str(received_dict['HEIGHT'])
                    print("check this received camera id== ", camera_id)
                    ...
                    if camera_id==0:
                        ...
                        command_runner_threaded(f"gst-launch-1.0 v4l2src device=\"/dev/video2\" ! video/x-raw, width=160, height=120, framerate=10/1 ! videoconvert ! 'video/x-raw, format=I420, width=160, height=120' ! omxh265enc bitrate=40 iframeinterval=5 EnableStringentBitrate=true control-rate=4 temporal-tradeoff=4 EnableTwopassCBR=true ! 'video/x-h265, stream-format=byte-stream' ! h265parse ! queue ! rtph265pay pt=127 ! udpsink host=10.0.0.2 port=6000 sync=false v4l2src device=\"/dev/video3\" ! video/x-raw, width=160, height=120, framerate=10/1 ! videoconvert ! 'video/x-raw, format=I420, width=160, height=120' ! omxh265enc bitrate=40 iframeinterval=5 EnableStringentBitrate=true control-rate=4 temporal-tradeoff=4 EnableTwopassCBR=true ! 'video/x-h265, stream-format=byte-stream' ! h265parse ! queue ! rtph265pay pt=127 ! udpsink host=10.0.0.2 port=6001 sync=false", stop_on=trigger_function, on_exit=feed_ended, priority='low', io_priority='low',shell=False)

                    print("camra id running: now",camera_id)
                else:
