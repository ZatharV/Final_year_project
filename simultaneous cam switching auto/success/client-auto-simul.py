...
def main():
    ...
    command_runner_threaded(f'gst-launch-1.0 udpsrc port={args.video_port} ! application/x-rtp,encoding-name=H265,payload=127 ! rtph265depay ! h265parse ! queue ! omxh265dec ! videoconvert ! videoscale ! \'video/x-raw, format=I420, width={args.width}, height={args.height}\' ! nveglglessink window-x=0 window-y=0 max-lateness=-1 enable-last-sample=False udpsrc port={args.video_port + 1} ! application/x-rtp,encoding-name=H265,payload=127 ! rtph265depay ! h265parse ! queue ! omxh265dec ! videoconvert ! videoscale ! \'video/x-raw, format=I420, width={args.width}, height={args.height}\' ! nveglglessink window-x=1280 window-y=0 max-lateness=-1 enable-last-sample=False', stop_on=trigger_function, on_exit=feed_ended, priority='low', io_priority='low',shell=False)
    ...
    while True:
        ...
        try:
            ...
            if (camera_id == 0):
                ...
                command_runner_threaded(f'gst-launch-1.0 udpsrc port={args.video_port} ! application/x-rtp,encoding-name=H265,payload=127 ! rtph265depay ! h265parse ! queue ! omxh265dec ! videoconvert ! videoscale ! \'video/x-raw, format=I420, width={args.width}, height={args.height}\' ! nveglglessink window-x=0 window-y=600 max-lateness=-1 enable-last-sample=False udpsrc port={args.video_port + 1} ! application/x-rtp,encoding-name=H265,payload=127 ! rtph265depay ! h265parse ! queue ! omxh265dec ! videoconvert ! videoscale ! \'video/x-raw, format=I420, width={args.width}, height={args.height}\' ! nveglglessink window-x=1280 window-y=600 max-lateness=-1 enable-last-sample=False', stop_on=trigger_function, on_exit=feed_ended, priority='low', io_priority='low',shell=False)
            print("cmd executed")
        except socket.error:
