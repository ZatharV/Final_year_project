...
def main():
    ...
    command_runner_threaded(f"gst-launch-1.0 v4l2src device=\"/dev/video{0}\" ! video/x-raw, width=160, height=120, framerate=10/1 ! videoconvert ! 'video/x-raw, format=I420, width=160, height=120' ! omxh265enc bitrate=40 iframeinterval=5 EnableStringentBitrate=true control-rate=4 temporal-tradeoff=4 EnableTwopassCBR=true ! 'video/x-h265, stream-format=byte-stream' ! h265parse ! queue ! rtph265pay pt=127 ! udpsink host=10.0.0.2 port=6000 sync=false -e", stop_on=trigger_function, on_exit=feed_ended, priority='low', io_priority='low',shell=False)
    while True:
        ...
        try:
            ...
            while True:
                ...
                if data:
                    ...
                    camera_id = str(received_dict['ID'])
                    ...
                    while(True == wait_end_feed):
                        pass
                    command_runner_threaded(f"gst-launch-1.0 v4l2src device=\"/dev/video{camera_id}\" ! video/x-raw, width=160, height=120, framerate=10/1 ! videoconvert ! 'video/x-raw, format=I420, width=160, height=120' ! omxh265enc bitrate=40 iframeinterval=5 EnableStringentBitrate=true control-rate=4 temporal-tradeoff=4 EnableTwopassCBR=true ! 'video/x-h265, stream-format=byte-stream' ! h265parse ! queue ! rtph265pay pt=127 ! udpsink host=10.0.0.2 port=6000 sync=false -e", stop_on=trigger_function, on_exit=feed_ended, priority='low', io_priority='low',shell=False)
