            ...
            while True:
                ...
                if data:
                    ...
                    while(True == wait_end_feed):
                        pass
                    command_runner_threaded(f"gst-launch-1.0 v4l2src device=\"/dev/video{camera_id}\" ! video/x-raw, width=320, height=240, framerate=10/1 ! videoconvert ! 'video/x-raw, format=I420, width=320, height=240' ! omxh265enc bitrate=40 iframeinterval=5 EnableStringentBitrate=true control-rate=4 temporal-tradeoff=4 EnableTwopassCBR=true ! 'video/x-h265, stream-format=byte-stream' ! h265parse ! queue ! rtph265pay pt=127 ! udpsink host=10.0.0.2 port=6000 sync=false -e", stop_on=trigger_function, on_exit=feed_ended, priority='low', io_priority='low',shell=False)
